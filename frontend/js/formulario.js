window.onload = () => {
    ...
    let peliID
    let elegir = confirm('deseas editar o eliminar una pelicula? si seleciona cancel le abilitara a crear peli')
    if (elegir == true) {
        let msg = prompt('escriba le id de la peli que desea modificar')
        if (msg) {
            peliID = +msg
        }
    }
    ...
    let cargarDato = async (data) => {
        ...
        btnEditar.onclick = async (e) => {
            ...
            if (confirmar) {
                ...
                alert('la pelicula se actualizo correctamente ' + result)
            }
        }
        ...
        btnEliminar.onclick = async (e) => {
            ...
            e.preventDefault();
            let respuesta = confirm('estas seguro que desea continuar?')
